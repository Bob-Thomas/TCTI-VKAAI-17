...
train_set, valid_set, test_set = mnist_loader.load_data_wrapper()

network = make_network([784, 30, 10])
...
for i in range(1000):
    ...
    for x in range(len(train_set)):
        deltas = backprop(train_set[x][0], train_set[x][1], network, relu, derived_relu, 0.5)
        result = forward(train_set[x][0], network, relu)
        cumulative_err += ((result - train_set[x][1]) * (result - train_set[x][1]))
        for y in range(len(network)):
            network[y] = network[y] + deltas[y]
    print(forward(test_set[0][0], network, relu), test_set[0][1])
